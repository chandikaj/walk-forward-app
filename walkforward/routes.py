...
from walkforward import app, daily_returns, pvals, X_test_default, dest, session
...
def upload():
    ...
    if form.validate_on_submit:
        if form.data_files.data:
            ...
            category = True
            session['graph_file_name'] = 'optimal_graph_' + \
                secrets.token_hex(6) + '.json'

            session['para_file_name'] = 'parameters_' + \
                secrets.token_hex(6) + '.txt'
...
def graphs():
    ...
    if daily_returns is None:
        ...
        path = os.path.join(dest, session['graph_file_name'])
        option_list = []
        ...
        if request.form.get('Download_output') == 'Download_output':
            ...
            fitness_function = int(selected_optimal.split('_')[0][2])
            ...
            if fitness_function == 1:
                master_func_tradesT(
                    in_period, out_period, daily_returns, pvals, dest, un_anchored_check)
            else:
                master_func_tradesF(
                    in_period, out_period, fitness_function, daily_returns, pvals, un_anchored_check)
        ...
        if request.form.get('Create_Graph') == 'Create_Graph':
            ...
            with open(os.path.join(dest, session['para_file_name']), 'w') as f:
                for item in parameters:
                    f.write(str(item) + '\n')

            multichart, single_charts = make_plotly_plots(in_period, out_period, daily_returns, un_anchored_check, session['graph_file_name'],
            ...
            if request.method == 'POST':
                with open(os.path.join(dest, session['para_file_name']), 'r') as f:
                    para = f.readlines()

                in_period, out_period, un_anchored_check = [
                    w.strip('\n') for w in para]

                optimal_function = request.form.get('optimal')
                best_lines = make_plotly_plots(int(in_period), int(out_period), daily_returns, eval(un_anchored_check),
                                               session['graph_file_name'], optimal=optimal_function, create_best=True)
                ...
                return redirect(url_for('optimal_graph'))


@app.route('/optimalgraph')
def optimal_graph():
    path = os.path.join(dest, session['graph_file_name'])
    option_list = []
    ...
    return render_template('graphs_second.html', plot_6=figure, option_list=option_list)
